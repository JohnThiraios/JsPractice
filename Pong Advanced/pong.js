...
var Game = {
  ...
  endGameMenu: function (text) {
    ...
    // Draw the end game text (Game over or Winner)
  ...
  // Update all objects (Player/Ball/Ai/Speed)
  update: function () {
    if (!this.over) {
      ...
      // On New serve, move the ball to correct side and randomize the direction
      ...
      // If player collides with the bound Limits,update x and y coordinates
    ...
    // Check to see if Player won the round
    if (this.player.score === rounds[this.round]) {
      // Check if its last round, Display victory screen if it is
      if (!rounds[this.round + 1]) {
        ...
        // If there is another round,reset all values and increase round number
    ...
    // Check to see if AI won the round
  ...
  draw: function () {
    ...
    // Set the default canvas font and align to center
    ...
    // Draw the Winning score on the Center
    this.context.fillText(
      "Round " + (Pong.round + 1),
      this.canvas.width / 2,
      35
  ...
  loop: function () {
    ...
    // If game is not over, draw next frame
    if (!Pong.over) requestAnimationFrame(Pong.loop);
  },

  listen: function () {
    document.addEventListener("keydown", function (key) {
      // Handle the Press any button to begin Function to start game
      ...
      // Handles down arrows and S key events
      if (key.keyCode === 40 || key.keyCode === 83)
        Pong.player.move = DIRECTION.DOWN;
    });

    // Stops player Movement when no keys are being pressed
  ...
  // Wait for the delay to pass after each turn
  _turnDelayIsOver: function () {
    return new Date().getTime() - this.timer >= 1000;
  },

  // Select a random color as background for each round
